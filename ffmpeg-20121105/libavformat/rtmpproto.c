...
static int rtmp_open(URLContext *s, const char *uri, int flags)
{
	...
	else
	{
		...
		if (rt->listen)
			ff_url_join(buf, sizeof(buf), "tcp", NULL, hostname, port,"?listen&listen_timeout=%d",rt->listen_timeout * 1000);
		else
			ff_url_join(buf, sizeof(buf), "tcp", NULL, hostname, port, NULL);
	}

	if ((ret = ffurl_open(&rt->stream, buf, AVIO_FLAG_READ_WRITE, &s->interrupt_callback, &opts)) < 0)
	{
		av_log(s , AV_LOG_ERROR, "Cannot open connection %s\n", buf);
		goto fail;
	}
	...
	if (!rt->playpath)
	{
		...
		if (!strchr(fname, ':') && len >= 4 &&(!strcmp(fname + len - 4, ".f4v") ||!strcmp(fname + len - 4, ".mp4")))
		{
	...
	if (!rt->tcurl)
	{
		...
		ff_url_join(rt->tcurl, TCURL_MAX_LENGTH, proto, NULL, hostname,port, "/%s", rt->app);
	}
	...
	if (!rt->flashver) 
	{
		...
		if (rt->is_input) 
		{
			snprintf(rt->flashver, FLASHVER_MAX_LENGTH, "%s %d,%d,%d,%d", RTMP_CLIENT_PLATFORM, RTMP_CLIENT_VER1, RTMP_CLIENT_VER2,RTMP_CLIENT_VER3, RTMP_CLIENT_VER4);
		}
		else 
		{
			snprintf(rt->flashver, FLASHVER_MAX_LENGTH, "FMLE/3.0 (compatible; %s)", LIBAVFORMAT_IDENT);
		}
	}
...
static int rtmp_write(URLContext *s, const uint8_t *buf, int size)
{
	...
	do
	{
		...
		if (rt->flv_header_bytes < 11) 
		{
			...
			if ((ret = ff_rtmp_packet_create(&rt->out_pkt, RTMP_SOURCE_CHANNEL,pkttype, ts, pktsize)) < 0)
				return ret;
	...
	else if (ret == 1)
	{
		...
		if ((ret = ff_rtmp_packet_read_internal(rt->stream, &rpkt, rt->in_chunk_size, rt->prev_pkt[0], c)) <= 0)
			return ret;
