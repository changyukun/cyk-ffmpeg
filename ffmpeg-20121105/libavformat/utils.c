...
*/
...
#if 1 // --- cyk add
	if(s->pb==NULL)
		av_log(NULL, AV_LOG_INFO, "=cyk==> (s->pb=NULL)\n");
	else
		av_log(NULL, AV_LOG_INFO, "=cyk==> (s->pb!=NULL)\n");
#endif

	/* 在ffplay 播放器中，此值为空，s->pb = NULL*/
	if (s->pb) 
	{
	...
	/* 在ffplay 播放器中，此值s->iformat = NULL*/
	if ( (s->iformat && s->iformat->flags & AVFMT_NOFILE) ||(!s->iformat && (s->iformat = av_probe_input_format2(&pd, 0, &score)))) 
		return 0;

#if 1 // --- cyk add
	if(s->iformat==NULL)
		av_log(NULL, AV_LOG_INFO, "=cyk==> (s->iformat=NULL)\n");
	else
		av_log(NULL, AV_LOG_INFO, "=cyk==> (s->iformat!=NULL)\n");
#endif
	
	if ((ret = avio_open2(&s->pb, filename, AVIO_FLAG_READ | s->avio_flags,&s->interrupt_callback, options)) < 0)
		return ret;
...
*/
	...
	if ((ret = init_input(s, filename, &tmp)) < 0) /* 进入函数分析*/
		goto fail;
