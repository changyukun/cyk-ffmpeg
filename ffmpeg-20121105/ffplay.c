...
*/
	...
	ic = avformat_alloc_context(); /* 分配AVFormatContext  内存空间，最重要的一个数据结构*/
	ic->interrupt_callback.callback = decode_interrupt_cb; /* 设定回调函数到ffmpeg  中*/
	ic->interrupt_callback.opaque = is;

	err = avformat_open_input(&ic, is->filename, is->iformat, &format_opts); /* 此时 is->iformat = NULL*/
	if (err < 0) 
	{
	...
	for (;;) 
	{
		...
		if (eof)
		{
			...
			if (is->audio_stream >= 0 &&is->audio_st->codec->codec->capabilities & CODEC_CAP_DELAY) 
			{
				av_init_packet(pkt);
				pkt->data = NULL;
				pkt->size = 0;
...
*/
	...
	is->read_tid     = SDL_CreateThread(read_thread, is);/* 创建解码线程*/
	if (!is->read_tid) 
	{
		av_free(is);
		return NULL;
...
*/
	...
	fill_rectangle(screen, is->xleft, is->ytop, is->width, is->height, bgcolor, 1);
}
